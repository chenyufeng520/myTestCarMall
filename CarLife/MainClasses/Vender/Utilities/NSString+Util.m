...
{
    NSString *tmp = [self stringByReplacingOccurrencesOfString:@"T" withString:@" "];
    return [tmp substringWithRange:NSMakeRange(0, 19)];
}
...
+(NSString *)dateStringWithFmt:(NSString *)fmt withDate:(NSString *)date{
   // NSMutableString *mstr=[NSMutableString stringWithString:date];
   // date=[date stringByReplacingOccurrencesOfString:@"T" withString:@" "];
   // date=[date substringToIndex:19];
    
    NSDateFormatter *dfmt=[[NSDateFormatter alloc]init];
    dfmt.dateFormat=@"yyyy-MM-dd HH:ss:mm";
    NSDate *tempdate=[dfmt dateFromString:date];
    dfmt.dateFormat=fmt;
    return  [dfmt stringFromDate:tempdate];
}
@end
