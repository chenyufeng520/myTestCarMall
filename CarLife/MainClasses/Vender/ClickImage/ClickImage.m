...
UIViewController* goBackDelegate;
...
- (void)showImage:(UIImageView *)defaultImageView{
    ...
    if (clickDelegate!=nil) {
            ...
            [UIView animateWithDuration:(_duration/2) animations:^{
                ...
                for (UIView* next = [self superview]; next; next = next.superview) {
                    ...
                    if ([nextResponder isKindOfClass:[UIViewController class]]) {
                        [((UIViewController*)nextResponder) presentViewController:clickDelegate animated:NO completion:^{
                            ;
                        }];//可更改为UINavigation推入
                    }
                }
                goBackDelegate = clickDelegate;
            }];
        }];
    } else {
        ...
        [UIView animateWithDuration:_duration animations:^{
            ...
            goBackgroundView.backgroundColor=[UIColor blackColor];//[UIColor colorWithRed:0 green:0 blue:0 alpha:0.5];
...
+ (void)dismissClickView {
    UIView *snapView = goBackDelegate.view;
    if(UIGraphicsBeginImageContextWithOptions != NULL)
    {
        UIGraphicsBeginImageContextWithOptions(snapView.frame.size, NO, 0.0);
    } else {
    ...
    [goBackDelegate dismissViewControllerAnimated:NO completion:^{
        ;
    }];
